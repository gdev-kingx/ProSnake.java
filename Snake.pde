...
int[]x_direction = {0, 0, 1, -1}, y_direction = {1, -1, 0, 0}; // direction of x and y
...
void draw() {
  background(10);
  fill(0, fc2, 0); //Snake Color Green
  ...
  if (!gameOver) {
    ...
    if (frameCount%speed == 0) {
      ...
      if (x.get(0) == foodx && y.get(0) == foody) { //new food if we touch
        if (x.size() % 5 == 0 && speed >= 2) speed -= 1; // every 5 pts speed increases
